...
const SwipeableMessage = ({
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (allowedDirection) {
      ...
      if (
        Math.abs(deltaX) > REPLY_TRIGGER_THRESHOLD &&
    ...
    <div className={`relative ${className}`}>
      ...
      {showReplyIcon && (
        <div
          ...
          style={{
            opacity: Math.min(
              Math.abs(translateX) / REPLY_TRIGGER_THRESHOLD,
              1,
            ),
          }}
        >
      ...
      {!isDragging && (
        <div
          className={`absolute top-1/2 transform -translate-y-1/2 text-xs text-gray-500 opacity-0 hover:opacity-100 transition-opacity duration-300 ${
